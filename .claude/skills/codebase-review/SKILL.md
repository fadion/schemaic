...
| **A5** | Test map | Decision functions vs. tests per module; logic in UI/app that belongs in core; and whether the invariant-enforcing tests still enforce — the DDL round-trip gate, the 1-row write-back net, export `*_to` byte-equality, and `intel`'s two-quote-table agreement. **"Lexer agreement" used to be listed here as a fourth and did not exist**, under that name or any other: the "one SQL boundary lexer" invariant is held by convention and review alone, with no source gate scanning for a hand-rolled quote/comment scanner. Do not re-verify a gate this table names — check it is there first. |
