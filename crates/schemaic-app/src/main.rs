...
fn main() -> std::process::ExitCode {
    ...
    // Headless CLI mode — `schemaic list` / `databases` / `query` / `exec` / `version` /
    // `help`. Like
    ...
    let inline_ai_run: Rc<dyn Fn(InlineAiRequest)> = {
        ...
        Rc::new(move |req: InlineAiRequest| {
            ...
            let plan = match ai::inline_plan(
                ...
                &intent,
                &system,
                // Ctrl+K is over the editor, so both levers are real here.
        ...
        Rc::new(
            move |req: schemaic_ui::AiFillRequest, done: schemaic_ui::AiFillDoneFn| {
                ...
                handle.spawn(async move {
                    ...
                    // A grid context menu: no editor, no query. See
                    // `schemaic_ai::SAMPLE_LEVERS`.
                    let res = match ai::inline_plan(
                        harness,
                        &cli_path,
                        &model,
                        &effort,
