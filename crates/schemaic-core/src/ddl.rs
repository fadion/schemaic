...
fn pg_column_clauses(from: &ColumnInfo, to: &ColumnInfo, d: SqlDialect) -> Vec<String> {
    ...
    if from.auto_increment != to.auto_increment {
        out.push(if to.auto_increment {
            let kind = if to.identity_always {
                "ALWAYS"
            } else {
                "BY DEFAULT"
            };
            format!("ALTER COLUMN {name} ADD GENERATED {kind} AS IDENTITY")
        } else {
            format!("ALTER COLUMN {name} DROP IDENTITY IF EXISTS")
        });
    } else if from.identity_always != to.identity_always && to.auto_increment {
        // **The identity's *kind*, which had no arm at all.** `columns_equal`
        // compares `identity_always`, so a `GENERATED ALWAYS` against a
        // `GENERATED BY DEFAULT` is reported Differing — and with nothing here
        // the change set was non-empty and emitted nothing: the preview listed
        // one change over an empty SQL box, Apply reported "Applied 0
        // statements to 1 object", and the next compare found the same
        // difference. A sync that never converged.
        //
        // Only when the column still *is* an identity; the arm above already
        // carries the kind when one is being added, and dropping one has no
        // kind to set.
        out.push(format!(
            "ALTER COLUMN {name} SET GENERATED {}",
            if to.identity_always {
                "ALWAYS"
            } else {
                "BY DEFAULT"
            }
...
pub fn supports_change(dialect: SqlDialect, change: &Change) -> bool {
    ...
    if let Change::AlterColumn { from, to, .. } = change
        ...
        && to.generated.is_some()
        && from.generated_stored != to.generated_stored
    {
        return false;
    }
...
mod tests {
    ...
    /// **And a change that is raised has to be a statement or a refusal**, which
    /// the test above does not ask: it asserts only `!cs.changes.is_empty()`.
    ///
    /// `columns_equal` compares `generated_stored`, so a MySQL flip raises an
    /// `AlterColumn` — and `ColumnInfo::definition_sql` writes the
    /// `VIRTUAL`/`STORED` keyword only for PostgreSQL and SQLite, so the emitted
    /// `MODIFY COLUMN g … GENERATED ALWAYS AS (…)` carries none. MySQL reads the
    /// keywordless form as `VIRTUAL`: the `STORED → VIRTUAL` direction silently
    /// **un-materialises** the column, and the reverse either does nothing or is
    /// refused by the server. Withheld now, so `unsupported()` puts it in front
    /// of the user with the rest of the plan intact.
    #[test]
    fn a_mysql_stored_flip_is_withheld_rather_than_emitted_without_the_keyword() {
        let base = |stored: bool| TableInfo {
            name: "t".into(),
            columns: vec![
                ColumnInfo {
                    name: "a".into(),
        ...
        for (from, to) in [(true, false), (false, true)] {
            let current = base(from);
            let draft = TableDraft::from_table(&base(to));
            let cs = diff(&current, &draft, MySql);
            assert!(!cs.changes.is_empty(), "{from} -> {to}: nothing raised");
            assert!(
                !cs.unsupported().is_empty(),
                "{from} -> {to}: emitted without the keyword instead of being \
                 withheld — {:?}",
                cs.emit()
            );
        }
        ...
        for d in [Postgres, Sqlite] {
            ...
            let draft = TableDraft::from_table(&base(false));
            assert!(
