...
pub struct Ui {
    ...
    /// Settings → General → Command line: put the `schemaic` command on `PATH`,
...
mod whole_ui_gate {
    ...
    const BUDGET: &[(&str, usize)] = &[
        ...
        // — `theme_settings_overlay(LayoutUi, open_config_dir, CliCommand)`, the
        // last two General's trips outside the app — and
