...
fn stats_body(
    ...
    let content: AnyView = match state {
        ...
        PropertiesState::Failed(e) => v_stack((
            note_line(icons::TRIANGLE_ALERT, theme::error, e),
            structure_section(target, info),
        ))
        .style(|s| s.flex_col().gap(16.0).width_full())
        .into_any(),
        PropertiesState::Unsupported => v_stack((
            note_line(
        ...
        .style(|s| s.flex_col().gap(16.0).width_full())
        .into_any(),
        PropertiesState::Loaded(stats) if !stats.has_any() => v_stack((
            note_line(
                icons::CIRCLE_QUESTION,
                theme::text_dim,
                if target.is_view {
        ...
        .style(|s| s.flex_col().gap(16.0).width_full())
        .into_any(),
        PropertiesState::Loaded(stats) => v_stack((
            headline(&stats),
...
/// nothing — a `COUNT(*)` needs no catalogue. Hidden only once an exact count is
/// in hand, since pressing it again would re-scan the table to print the same
/// number.
fn count_row(
    target: &PropertiesTarget,
    ...
    let control: AnyView = if counted {
        empty().into_any()
    } else if counting {
        loading_dots("Counting", theme::text_dim, FONT_LABEL).into_any()
    } else {
        ...
        in_ring_button(face, ring, COUNT_TAB, true, 6.0, move || (run)(t.clone()))
    };

    let hint: AnyView = match (&count_err, counted) {
        (Some(e), _) => text(e.clone())
            .style(|s| s.font_size(FONT_HINT).color(theme::error()))
            .into_any(),
        (None, true) => text("Counted just now — an exact figure, as of when it ran.")
            .style(|s| s.font_size(FONT_HINT).color(theme::text_faint()))
            .into_any(),
        (None, false) => text("A full scan of the table. Slow on a large one.")
            .style(|s| s.font_size(FONT_HINT).color(theme::text_faint()))
            .into_any(),
    };

    h_stack((control, hint))
        .style(|s| s.items_center().gap(10.0).width_full())
        .into_any()
}

/// Where the bytes went: a proportional bar over a legend.
fn storage_section(stats: &TableStats) -> AnyView {
    let Some((data, index, free)) = stats.storage_split() else {
        return empty().into_any();
    };
    ...
    .style(|s| s.flex_col().gap(9.0).width_full())
    .into_any()
}
...
fn structure_section(target: &PropertiesTarget, info: Option<&TableInfo>) -> AnyView {
...
/// designer.
fn options_section(stats: &TableStats, info: Option<&TableInfo>) -> AnyView {
    let mut rows: Vec<AnyView> = Vec::new();
...
fn vacuum_note(stats: &TableStats) -> AnyView {
...
/// already know.
fn index_section(stats: &TableStats) -> AnyView {
...
fn index_row(idx: &IndexStats) -> AnyView {
    ...
    let detail = text(facts.join(" · ")).style(|s| s.font_size(FONT_HINT).color(theme::text_dim()));

    let flag: AnyView = if unused {
        h_stack((
            ...
            text("never used since the server's counters were reset")
                .style(|s| s.font_size(FONT_HINT).color(theme::plan_warn())),
        ))
    ...
    v_stack((
        h_stack((name, detail)).style(|s| s.items_center().gap(10.0).width_full()),
        flag,
    ))
...
fn freshness_note(stats: &TableStats) -> AnyView {
    match stats.freshness.note() {
        Some(n) => note_line(icons::CIRCLE_QUESTION, theme::text_faint, n),
        None => empty().into_any(),
    }
}
...
fn section(title: &'static str, rows: Vec<AnyView>) -> AnyView {
    v_stack((
        text(title).style(|s| s.font_size(FONT_LABEL).font_bold().color(theme::text_dim())),
        v_stack_from_iter(rows).style(|s| s.flex_col().gap(4.0).width_full()),
    ))
...
fn detail(label: &'static str, value: String) -> AnyView {
    h_stack((
        ...
        text(value).style(|s| s.font_size(FONT_BODY).color(theme::text())),
    ))
...
fn note_line(icon: &'static str, color: fn() -> Color, message: String) -> AnyView {
    h_stack((
        ...
        text(message).style(move |s| s.font_size(FONT_HINT).color(color())),
    ))
